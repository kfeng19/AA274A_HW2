...
def optimize_trajectory(time_weight=1.0, verbose=True):
    ...
    def cost(z):
        ...
        return t_f * time_weight + t_f * np.sum(u ** 2, axis=None)

    def constraints(z):
        t_f, s, u = unpack_decision_variables(z)
        dt = t_f / N
        l_constr = [s[0] - s_0, s[-1] - s_f]
        # l_constr = []
        for i in range(N):
            l_constr.append(s[i+1] - (s[i] + dt * dynamics(s[i], u[i])))
        return np.concatenate(l_constr)

    t_f_guess = 1
    s_guess = np.linspace(s_0, s_f, N+1)
    u_guess = np.ones((N, 2))
    z_guess = pack_decision_variables(t_f_guess, s_guess, u_guess)
